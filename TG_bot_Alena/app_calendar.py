...
class AppointmentCalendar:
    ...
    async def get_time_slots_keyboard(self, date: str) -> InlineKeyboardMarkup:
        ...
        all_slots = self.db.get_all_time_slots()
        ...
        active_slots = sorted([s for s in all_slots if s['is_active']], key=lambda x: x['time'])
        
        # Разбиваем слоты по 3 в ряд для удобства
        for i in range(0, len(active_slots), 3):
            row = []
            for slot in active_slots[i:i+3]:
                time = slot['time']
                if time in busy_slots:
                    # Занятый слот - с крестиком
                    row.append(InlineKeyboardButton(
                        text=f"❌ {time}",
                        callback_data="ignore"
                    ))
